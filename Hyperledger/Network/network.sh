...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create MainOrg Identities ######################"
    echo "##########################################################"
    ...
    echo "##########################################################"
    echo "############ Create RawMaterialOrg Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-rawmaterialorg.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
    ...
    echo "##########################################################"
    echo "############ Create ProcessorOrg Identities ######################"
    echo "##########################################################"
    ...
    echo "##########################################################"
    echo "############ Create ProducerOrg Identities ######################"
    echo "##########################################################"
    ...
    echo "##########################################################"
    echo "############ Create DistributorOrg Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-distributororg.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
    ...
    echo "##########################################################"
    echo "############ Create AssemblerOrg Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-assemblerorg.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
  ...
  echo
  echo "Generate CCP files for MainOrg and RawMaterialOrg"
